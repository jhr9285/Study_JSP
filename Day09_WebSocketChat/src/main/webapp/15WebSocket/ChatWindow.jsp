...
<html>
	<head>
		...
		<script>
			...
			// 채팅창이 열리면(페이지 로딩이 끝나면) 
			// 대화창, 메시지 입력창, 대화명 표시란으로 사용할 DOM 객체 저장
			...
			function sendMessage() {
				// 대화창에 표시
				chatWindow.innerHTML += "<div class='myMsg'>" + chatMessage.value + "</div>"; // 내가 보낸 메시지 표시
				webSocket.send(chatId + '|' + chatMessage.value); // 서버로 전송
				chatMessage.value = "";  // 메시지 입력창 내용 지우기
				chatWindow.scrollTop = chatWindow.scrollHeight; // 대화창 스크롤 (★★★스크롤이 자동으로 내려가게 해주는 코드)
			    // scrollTop : 맨 위부터 스크롤한 위치까지의 값을 구하는 속성
			    // scrollHeight : 스크롤한 높이?
			}
